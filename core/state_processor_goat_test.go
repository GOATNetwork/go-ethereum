...
func TestProcessGoatRequests(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: []goattypes.Request{
				goattypes.NewGasRequest(100, big.NewInt(1e9)),
				&goattypes.WithdrawalRequest{Id: 100, Amount: 10, TxPrice: 1, Address: "1FwtBCsUqhQ8Zshw1or4aSUfdk1UDi83vh"},
				&goattypes.ReplaceByFeeRequest{Id: 1, TxPrice: 10},
				&goattypes.Cancel1Request{Id: 10},
				&goattypes.AddVoterRequest{
				...
				&goattypes.RemoveVoterRequest{Voter: common.HexToAddress("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4")},
				&goattypes.CreateRequest{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != len(tt.want) {
				t.Errorf("ProcessGoatRequests() = %v, want %v", len(got), len(tt.want))
				return
			}

			for idx, req := range tt.want {
				if raw := req.Encode(); !bytes.Equal(raw, got[idx]) {
					t.Errorf("ProcessGoatRequests() idx %d: not equal: want %#v", idx, req)
					return
				}
				cpy := req.Copy()
				if err := cpy.Decode(got[idx]); err != nil {
					t.Errorf("ProcessGoatRequests() idx %d decode error = %v", idx, err)
					return
				}
				if !reflect.DeepEqual(cpy, req) {
					t.Errorf("ProcessGoatRequests() copy = %#v  want = %#v", cpy, req)
					return
				}
			}
		})
...
func TestProcessGoatGasFee(t *testing.T) {
	...
	requestsHash := types.CalcRequestsHash([][]byte{goattypes.NewGasRequest(1, rwBalace.ToBig()).Encode()})
